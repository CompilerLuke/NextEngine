...
FV_Mesh build_mesh(CFDVolume& mesh) {
    ...
    for (uint i = 0; i < mesh.cells.length; i++) {
        ...
        for (uint f = 0; f < shape.num_faces; f++) {
            ...
            else if (face.neighbor.id > i) interior_count++;
        }
    }
    ...
    for (uint cell_id = 0; cell_id < mesh.cells.length; cell_id++) {
        ...
        for (uint f = 0; f < shape.num_faces; f++) {
            ...
            if (!boundary) {
                ...
                vec3 neigh_center = compute_centroid(mesh, neigh_cell.vertices, shape[neigh_cell.type].num_verts);
            ...
            data->center_to_face.row(face_id) = Eigen::Vector3d{center_to_face.x, center_to_face.y, center_to_face.z};
            data->normal.row(face_id) = Eigen::Vector3d{normal.x, normal.y, normal.z};
