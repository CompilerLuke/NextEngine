...
refl::Struct init_Flyover() {
	...
	type.fields.append({"past_movement_speed", offsetof(Flyover, past_movement_speed), refl::TypeResolver<array<3,glm::vec2>>::get() });
	return type;
}
...
void write_to_buffer(SerializerBuffer& buffer, Flyover& data) {
	...
	for (uint i = 0; i < data.past_movement_speed.length; i++) {
...
refl::Struct init_Terrain() {
	...
	type.fields.append({"heightmap_points", offsetof(Terrain, displacement_map), refl::TypeResolver<vector<float>>::get() });
...
void write_to_buffer(SerializerBuffer& buffer, Terrain& data) {
	...
	for (uint i = 0; i < data.displacement_map.length; i++) {
         write_to_buffer(buffer, data.displacement_map[i]);
    }
    write_to_buffer(buffer, data.max_height);
}
...
refl::Struct init_Entity() {
	...
	type.fields.append({"id", offsetof(Entity, id), refl::TypeResolver<uint>::get() });
