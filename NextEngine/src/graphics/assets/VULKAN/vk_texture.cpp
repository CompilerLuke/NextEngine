...
void transition_ImageLayout(VkCommandBuffer cmd_buffer, VkImage image, VkFormat format, VkImageLayout oldLayout,
                            VkImageLayout newLayout, VkImageAspectFlags aspect, int mips, int src_queue =
    ...
    if(aspect == 0) {
        printf("Warning specify aspect explicitly!");
...
VkImageAspectFlags to_vk_image_aspect_flags(TextureAspect aspect) {
    VkImageAspectFlags flags = {};
    if (aspect & TextureAspect::Color) flags |= VK_IMAGE_ASPECT_COLOR_BIT;
    if (aspect & TextureAspect::Depth) flags |= VK_IMAGE_ASPECT_DEPTH_BIT;
    if (aspect & TextureAspect::Stencil) flags |= VK_IMAGE_ASPECT_STENCIL_BIT;
    return flags;
}
...
VkImageUsageFlags to_vk_usage_flags(TextureUsage usage) {
	...
	if (usage & TextureUsage::DepthAttachment) usage_flags |= VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
...
VkFormat to_vk_image_format(TextureFormat format, TextureAspect aspect, uint num_channels) {
    if (format == TextureFormat::DEPTH) {
        return find_depth_format(rhi.device, aspect & TextureAspect::Stencil);
    }
...
VkFormat to_vk_image_format(const TextureDesc& desc) {
	return to_vk_image_format(desc.format, desc.aspect, desc.num_channels);
}
...
Texture make_TextureImage(TextureAllocator& allocator, const Image& image) {
	...
	if (info == NULL) {
		...
		info->format = image_format;
		info->image = vk_image;
        info->aspect = to_vk_image_aspect_flags(image.aspect);
	}
	else {
		vk_image = info->image;
		assert(false);
	...
	transition_ImageLayout(cmd_buffer, vk_image, image_format, VK_IMAGE_LAYOUT_UNDEFINED,
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, info->aspect, 1);
	copy_buffer_to_image(cmd_buffer, allocator.staging.buffer, vk_image, image.width, image.height, offset);
	transition_ImageLayout(cmd_buffer, vk_image, image_format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                           VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, info->aspect, 1, staging_queue
                           .queue_family, staging_queue.dst_queue_family);
	
	Texture result;
	result.desc = image;
	result.alloc_info = info;
	result.image = vk_image;
	result.view = make_ImageView(device, vk_image, image_format, info->aspect);
...
void transfer_image_ownership(TextureAllocator& allocator, VkCommandBuffer cmd_buffer) {
	...
	while (transfer_ownership) {
		...
		transition_ImageLayout(cmd_buffer, transfer_ownership->image, transfer_ownership->format, 
			VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, transfer_ownership->aspect,
            1, queue
            .queue_family, queue.dst_queue_family);
...
void transition_layout(CommandBuffer& cmd_buffer, texture_handle handle, TextureLayout from, TextureLayout to) {
    ...
    VkImageAspectFlags aspect = to_vk_image_aspect_flags(texture->desc.aspect);
	transition_ImageLayout(cmd_buffer, texture->image, format, to_vk_layout[(uint)from], to_vk_layout[(uint)to],
                           aspect, texture->desc.num_mips);
}
