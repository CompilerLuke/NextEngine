...
void make_AssetManager(string_view path, string_view engine_path) {
	...
	default_materials.missing = { 1 };
	{
		...
		desc.mat_channel3("diffuse", glm::vec3(1.0), default_textures.checker);
		desc.mat_channel1("metallic", 0.0f);
        desc.mat_channel1("roughness", 0.5f);
        desc.mat_channel1("normal", 1.0, default_textures.normal);
        desc.mat_vec2("tiling", glm::vec3(1.0));
...
Model::Model() {}
Model::Model(slice<Vertex> vertices, slice<uint> indices) {
    AABB aabb;
    for(int i = 0; i < vertices.length; i++) {
        aabb.update(vertices[i].position);
    }

    Mesh* mesh = PERMANENT_ALLOC(Mesh);
    mesh->lod_count = 1;
    mesh->vertices[0] = vertices;
    mesh->indices[0] = indices;
    mesh->buffer[0] = alloc_vertex_buffer<Vertex>(VERTEX_LAYOUT_DEFAULT, vertices, indices);
    mesh->aabb = aabb;

    this->aabb = aabb;
    this->meshes = {mesh,1}; //todo: make meshes RAII
}
...
void load_assets_in_queue() {
	...
	while (assets.equirectangular_to_cubemap_jobs.dequeue(&job)) {
        ...
        convert_equirectangular_to_cubemap(*assets.cubemap_pass_resources, job.env_map);
