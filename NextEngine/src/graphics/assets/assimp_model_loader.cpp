...
struct ModelLoadingScratch {
 ...
 void process_Mesh(ModelLoadingScratch* scratch, aiMesh* mesh) {
    ...
    if(mesh->mNormals == nullptr || mesh->mTangents == nullptr || mesh->mBitangents == nullptr) {
        fprintf(stderr, "Mesh is missing full normals!\n");
    }

    glm::vec3 rand = glm::normalize(glm::vec3(0.23,0.36,0.7));

	for (int i = 0; i < mesh->mNumVertices; i++) {
        auto to_vec3 = [](aiVector3D v) {
            return glm::vec3(v.x,v.y,v.z);
        };

		glm::vec3 position = to_vec3(mesh->mVertices[i]);
        glm::vec3 normal = mesh->mNormals ? to_vec3(mesh->mNormals[i]) : glm::vec3(0);
		glm::vec3 tangent = mesh->mTangents ? to_vec3(mesh->mTangents[i]) : glm::cross(normal,rand);
        glm::vec3 bitangent = mesh->mBitangents ? to_vec3(mesh->mBitangents[i]) : glm::cross(normal,tangent);

		auto first_coords = coords ? glm::vec2(coords[i].x, coords[i].y) : glm::vec2(0,0);

		vertices[i] = {
			apply_trans * glm::vec4(position, 1.0),
			apply_trans3 * normal,
			first_coords,
			apply_trans3 * tangent,
			apply_trans3 * bitangent
		};
...
void load_assimp(Model* model, string_view real_path, const glm::mat4& apply_transform) { //TODO WHEN REIMPORTED, can reuse same memory
	...
	if (real_path.ends_with(".fbx")) {
		...
		for (uint i = 0; i < MAX_MESH_LOD; i++) {
			...
			auto scene = importer[i].ReadFile(path.c_str(), aiProcess_Triangulate | aiProcess_GenSmoothNormals |
	...
	if (lods.length == 0) {
		auto scene = importer[0].ReadFile(real_path.c_str(), aiProcess_Triangulate | aiProcess_GenSmoothNormals |
    ...
    //todo: why use permanent allocator, allow meshes to be free!
    ...
    if(model->materials.length == 0) {
        ...
        model->materials.data = PERMANENT_ARRAY(sstring, 1);
        model->materials.data[0] = "material";
    } else {
