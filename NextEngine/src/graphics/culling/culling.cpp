...
void picking_subdivide_BVH(SubdivideBVHJob& job) {
	...
	if (job.mesh_count <= MAX_MESHES_PER_NODE || job.depth >= MAX_DEPTH) {
		...
		for (int i = 0; i < 2; i++) {
			...
			desc[count] = {picking_subdivide_BVH, jobs + i};
			count++;
		}
...
void assign_meshes_to_buckets(
    ...
    for (auto [e,trans,model_renderer,materials] : world.filter<Transform,ModelRenderer, Materials>(query)) {
        ...
        for (int mesh_index = 0; mesh_index < model->meshes.length; mesh_index++) {
            ...
            bucket.color_pipeline = query_pipeline(mat_handle, RenderPass::Scene, zprepass ? 1 : 0);
...
void build_acceleration_structure(ScenePartition& scene_partition, hash_set<MeshBucket, MAX_MESH_BUCKETS> &
        mesh_buckets, World& world, RenderSettings& render_settings) { //todo UGH THE CURRENT SYSTEM LIMITS HOW DATA
    // ORIENTED THIS CAN BE
    ...
    assign_meshes_to_buckets(world, mesh_buckets, aabbs, models_m, meshes, {STATIC}, render_settings.zprepass);
	...
	picking_subdivide_BVH(job);
}


#define MAX_DEPTH_CHECK_EACH 5

void cull_node(slice<CulledMeshBucket> culled, const ScenePartition& partition, const Node& node, int depth,
               glm::vec4 planes[6]) {
...
void cull_meshes(const ScenePartition& scene_partition, World& world, MeshBuckets& buckets, uint count,
                 slice<slice<CulledMeshBucket>> culled_mesh_bucket, Viewport viewports[], EntityQuery query,
                 RenderSettings& settings) {
	tvector<AABB> aabbs;
	tvector<glm::mat4> model_m;
	tvector<int> meshes;
	
	assign_meshes_to_buckets(world, buckets, aabbs, model_m, meshes, query.with_none(STATIC), settings.zprepass);
