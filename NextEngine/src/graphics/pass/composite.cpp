...
void make_composite_resources(CompositeResources& resources, texture_handle depth, texture_handle scene, texture_handle volumetric, texture_handle cloud, uint width, uint height) {
	...
	AttachmentDesc& attachment = add_color_attachment(framebuffer_desc, &resources.composite_map);
    attachment.usage = attachment.usage | TextureUsage::TransferSrc;

	add_dependency(framebuffer_desc, FRAGMENT_STAGE, RenderPass::Scene, TextureAspect::Color | TextureAspect::Depth);
	add_dependency(framebuffer_desc, FRAGMENT_STAGE, RenderPass::Volumetric, TextureAspect::Color);
...
void render_composite_pass(CompositeResources& resources, RenderSubmitCtx& submit) {
	RenderPass render_pass = submit.get_render_pass(RenderPass::Composite);
