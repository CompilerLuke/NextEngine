...
indices_length, uint* indices) {
	...
	u64 vert_size = self.layouts[layout].binding_desc.stride;
	u64 index_size = sizeof(uint);
	u64 vertices_size = vertices_length * vert_size;
	u64 indices_size = indices_length * index_size;
	...
	buffer.index_capacity = indices_length;
...
void make_VertexStreaming(VertexStreaming& self, VkDevice device, VkPhysicalDevice physical_device, StagingQueue&
queue, slice<LayoutVertexInputs> layouts, u64* vertices_size_per_layout, u64* indices_size_per_layout, u64 staging) {
	self.device = device;
	self.physical_device = physical_device;
	...
	//Allocate Buffers
	self.staging_buffer = make_HostVisibleBuffer(device, self.physical_device, 0, staging);
...
VertexBuffer alloc_vertex_buffer(VertexLayout vertex_layout, u64 vertices_length, void* vertices, u64 indices_length,
                                 uint* indices) {
