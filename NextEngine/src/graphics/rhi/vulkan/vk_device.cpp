...
QueueFamilyIndices find_queue_families(VkPhysicalDevice device, VkSurfaceKHR surface) {
	...
	for (int i = 0; i < queue_families.length; i++) {
		...
		uint transfer_ranking = graphics_support ? 100 : compute_support ? 2 : 3;
...
VkInstance make_Instance(const VulkanDesc& desc) {
    ...
    makeInfo.flags |= VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR;
	...
	if (vkCreateInstance(&makeInfo, nullptr, &result) != VK_SUCCESS) {
		fprintf(stderr, "failed to make instance!\n");
...
void make_logical_device(Device& device, const VulkanDesc& desc, VkSurfaceKHR surface) {	
	...
	if (vkCreateDevice(device.physical_device, &device_desc, nullptr, &device.device) != VK_SUCCESS) {
		fprintf(stderr, "failed to make logical device!\n");
