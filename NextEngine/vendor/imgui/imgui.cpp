...
void ImGui::RenderTextClippedEx(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    {
        draw_list->AddText(NULL, ImGui::GetCurrentContext()->FontSize, pos, GetColorU32(ImGuiCol_Text), text, text_display_end, 0.0f, NULL); //EDIT using context font size instead of draw list
    }
}
...
static ImRect ImGui::DockNodeCalcTabBarRect(const ImGuiDockNode* node)
{
    ...
    if (node->HasCollapseButton)
        r.Min.x += g.Style.FramePadding.x + 5; // + g.Style.ItemInnerSpacing.x; // <-- Adding ItemInnerSpacing makes the title text moves slightly when in a tab bar. Instead we adjusted RenderArrowDockMenu()
