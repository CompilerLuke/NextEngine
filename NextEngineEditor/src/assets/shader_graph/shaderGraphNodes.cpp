...
void node_option(string_view name, ShaderAsset* asset, glm::vec2 mouse_rel, bool& first, ShaderNode (make_node)()) {
	...
	bool enter_to_create_node = first && ImGui::IsKeyPressed((int)Key::Enter);
	bool click_to_create_node = ImGui::Button(tformat("New ", name, " Node").c_str());

	if (enter_to_create_node || click_to_create_node) {
		spawn_node(*asset->graph, mouse_rel, make_node());
...
void new_node_popup(ShaderAsset* asset) {
	...
	if (ImGui::BeginPopup("NewNode")) {
		...
		bool first = true;
		node_option("Texture", asset, position, first, make_texture_node);
		node_option("PBR", asset, position, first, make_pbr_node);
		node_option("Tex Coords", asset, position, first, make_tex_coords_node);
		node_option("Math", asset, position, first, make_math_node);
		node_option("Blend", asset, position, first, make_blend_node);
		node_option("Time", asset, position, first, make_time_node);
		node_option("Clamp", asset, position, first, make_clamp_node);
		node_option("Vec", asset, position, first, make_vec_node);
		node_option("Remap", asset, position, first, make_remap_node);
		node_option("Step", asset, position, first, make_step_node);
		node_option("Noise", asset, position, first, make_noise_node);
...
void ShaderCompiler::find_dependencies() {
	...
	for (int i = 0; i < graph.nodes_manager.slots.length; i++) {
		shader_node_handle handle = graph.nodes_manager.index_to_handle(i);
		...
		if (node->type == ShaderNode::TEXTURE_NODE) {
			...
			param.name = (string_view)get_dependency(handle); 
