...
bool load_scene(Editor& editor, const char** err) {
	...
	update_acceleration_structure(renderer.scene_partition, renderer.mesh_buckets, world, renderer.settings);
...
void register_callbacks(Editor& editor, Modules& _engine) {
	register_on_inspect_callbacks();

	editor.asset_tab.register_callbacks(editor.window, editor);
	editor.window.override_key_callback(override_key_callback);
	editor.window.override_char_callback(ImGui_ImplGlfw_CharCallback);
	editor.window.override_mouse_button_callback(override_mouse_button_callback);
}

Editor::Editor(Modules& modules, Application& game) :
    modules(modules),
	...
	actions{*this}
{
	...
	editor_viewport.contents = renderer.get_output_map();
    ...
    editor_viewport.viewport.width = modules.window->width;
    editor_viewport.viewport.height = modules.window->height;
...
void Editor::render() {
    bool is_fullscreen = playing_game && game_fullscreen;
    if (playing_game && is_fullscreen) {
        game.render();
        return;
    }

    float title_bar_margin = 50;
    ...
    if (is_fullscreen) {
        ...
        viewport = editor_viewport.viewport;
    }

    EntityQuery mask = playing_game ? EntityQuery().with_none(EDITOR_ONLY) : EntityQuery();

    GizmoRenderData gizmo_render_data;

    EntityQuery camera_mask = playing_game ? mask : EntityQuery{EDITOR_ONLY};
    RenderPrepareCtx prepare = renderer.extract_render_data(world, viewport, mask, camera_mask);
    extract_render_data_special_gizmo(gizmo_render_data, world, mask);

    RenderSubmitCtx submit = renderer.build_command_buffers(prepare);
	...
	//TEMPORARY SOLUTION TO TERRAIN INITIALIZATION PROBLEM
	{
		...
		if (receive_terrain == 0 && frame_index == 0) {
            ...
            if (auto has_terrain = world.first<Terrain>(); has_terrain) {
                auto[_, terrain] = *has_terrain;
                receive_generated_heightmap(terrain_resources, terrain);
                regenerate_terrain(world, terrain_resources, renderer.terrain_render_resources, EntityQuery{ EDITOR_ONLY }); //this should not be necessary
            }
			receive_terrain = 2;
		}
	}

	RenderPass scene_pass = submit.get_render_pass(RenderPass::Scene);
	if (!playing_game) {
		...
		render_special_gizmos(gizmo_resources, gizmo_render_data, scene_pass);
        if (selected_id != -1) {
            ID selected_id = this->selected_id;
            render_object_selected_outline(outline_selected, world, selected_id, scene_pass);
        }
        if (visibility & SHOW_PHYSICS) render_colliders(physics_resources, *scene_pass.cmd_buffer, world, EntityQuery());
        render_overlay(*this, scene_pass);
    } else {
        if (visibility & SHOW_PHYSICS) render_colliders(physics_resources, *scene_pass.cmd_buffer, world, mask);
    }

    RenderPass screen = submit.get_render_pass(RenderPass::Screen);
    if (playing_game && game_fullscreen) {
        Viewport viewport = editor_viewport.viewport; //prepare.get_viewport();
        begin_imgui(editor_viewport.input);
        ...
        if (ImGui::Begin("Playing Fullscreen")) {
            ImGui::Image(renderer.get_output_map(), ImVec2(viewport.width, viewport.height));
        } else {
            set_play_mode(*this, false);
        }
        ...
        end_imgui(*screen.cmd_buffer);
    } else {
        render_Editor(*this, screen, scene_pass);
    }


	renderer.submit_frame(submit);
    editor_viewport.input.clear();
...
void respond_to_framediffs(Editor& editor) {
	...
	if (rebuild_acceleration) {
		...
		build_acceleration_structure(renderer.scene_partition, renderer.mesh_buckets, editor.world, renderer.settings);
	}
...
Modules& Editor::get_modules() {
    return modules;
}

bool Editor::is_running() {
    return !window.should_close() && !exit;
}
...
void Editor::update() {
    ...
    if (editor.playing_game) {
        ...
        editor.game.get_modules().input = &update_ctx.input;
        editor.game.update();
    }
    else {
...
APPLICATION_API Application* init_app(Modules& modules, const char* args) {
    Dynamic_Application* application = PERMANENT_ALLOC(Dynamic_Application, modules, args, nullptr);
    return new Editor(modules, *application);
}