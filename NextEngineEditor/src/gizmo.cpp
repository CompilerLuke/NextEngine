...
material_handle make_icon_material(string_view path, bool billboard = true) {
	...
	mat_desc.mat_channel4("icon", glm::vec4(1), upload_Texture(image));
	free_Image(image);
...
void make_special_gizmo_resources(GizmoResources& resources) {
	{
		...
		mat_desc.mat_vec3("color", glm::vec3(1, 1, 0));
...

	{
		...
		mat_desc.mat_channel3("diffuse", glm::vec3(0,0,1));
...
void Gizmo::render(World& world, Editor& editor, Viewport& viewport, Input& input) {
    ...
    if(1) {
        ImGuizmo::SetRect(input.region_min.x, input.region_min.y, input.region_max.x - input.region_min.x,
                          input.region_max.y - input.region_min.y);
    }


    glm::mat4 proj = viewport.proj;
    glm::mat4 view = viewport.view;

	ImGuizmo::Manipulate(glm::value_ptr(view), glm::value_ptr(proj), guizmo_operation, guizmo_mode, glm::value_ptr
    (model_matrix), NULL, snap ? glm::value_ptr(snap_vec) : NULL);
