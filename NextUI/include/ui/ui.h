...
// todo: name ID -> Hash and GID -> ID, to make clear that ID are transient whereas GID is persistent
struct UI_ID {
    enum Type { Local, Global } type = Local;
    u64 hash = 0;
    ...
    UI_ID(Type type, UI_GID hash) : type(type), hash(hash) {}
};

inline bool operator==(UI_ID a, UI_ID b) { return a.type == b.type && a.hash == b.hash; }
...
inline Color color4(uint r, uint g, uint b, uint a = 1.0) {
    return glm::vec4(r/255.0f,g/255.0f,b/255.0f,a);
}

extern glm::vec4 shade1;
extern glm::vec4 shade2;
extern glm::vec4 shade3;
extern glm::vec4 border;
...
T& id(UI_ID id) { this->m_id = id; return *this; }
...
struct Empty : UIView {
    virtual LayedOutUIView& compute_layout(UI& ui, const BoxConstraint&);
};
...
struct GeometryView : UIView {
    std::function<void(glm::vec2 pos, glm::vec2 size)> change;

    LayedOutUIView& compute_layout(UI& ui, const BoxConstraint&) override;
};

UI_API UI* make_ui(struct Renderer&);
...
UI_API UI_GID find_or_create_guid(UI& ui, std::type_index type, UI_GID parent, uint32_t child_index, UI_ID id);
UI_API UI_GID find_or_create_guid(UI& ui, std::type_index type, UI_ID id);
...
T& get_state(UI& ui, UI_ID id = {}) {
    static hash_map<UI_GID, T, N> states;

    UIView& view = make_element<Empty>(ui);
    UI_GID guid = find_or_create_guid(ui, typeid(T), id);
    view.m_id = UI_ID(UI_ID::Global, guid);
