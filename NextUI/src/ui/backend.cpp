...
struct UIRenderer : RenderSystem {
    UIDrawData& data;

    UIRenderer(UIDrawData&);
    ~UIRenderer();
    ...
    void extract_render_data(RenderPrepareCtx&) override;
    void build_command_buffers(RenderSubmitCtx&) override;
};

UIRenderer::UIRenderer(UIDrawData& data) : data(data) {

    shader_handle shader = load_Shader("shaders/imgui_shader.vert", "shaders/imgui_shader.frag");

    sampler = query_Sampler({Filter::Linear, Filter::Linear});
    ...
    dummy_tex = default_textures.white;

    alloc_Arena(vertex_arena, MAX_UI_VERTEX_BUFFER_SIZE, &vertex_buffer, BUFFER_VERTEX);
    alloc_Arena(index_arena, MAX_UI_INDEX_BUFFER_SIZE, &index_buffer, BUFFER_INDEX);

    proj_ubo_buffer = alloc_ubo_buffer(sizeof(glm::mat4), UBO_PERMANENT_MAP);

    //CREATE DESCRIPTORS
    CombinedSampler sampler_array[MAX_UI_TEXTURES] = {};
    for (uint i = 0; i < MAX_UI_TEXTURES; i++) {
        sampler_array[i].sampler = sampler;
        sampler_array[i].texture = dummy_tex;
    }

    DescriptorDesc descriptor_desc;
    add_ubo(descriptor_desc, VERTEX_STAGE, proj_ubo_buffer, 0);
    ...
    for (uint i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        update_descriptor_set(descriptor[i], descriptor_desc);
    }
    ...
    pipeline = query_Pipeline(pipeline_desc);
}

UIRenderer::~UIRenderer() {
    dealloc_cpu_visible_buffer(vertex_buffer);
    dealloc_cpu_visible_buffer(index_buffer);
}

UIRenderer* make_ui_renderer(Renderer& renderer, UIDrawData& draw) {
    std::unique_ptr<UIRenderer> ui_renderer = std::make_unique<UIRenderer>(draw);
    UIRenderer* ptr = ui_renderer.get();
    renderer.add(std::move(ui_renderer));
    return ptr;
}
...
void UIRenderer::extract_render_data(RenderPrepareCtx &) {}
void UIRenderer::build_command_buffers(RenderSubmitCtx& submit) {
    RenderPass pass = submit.get_render_pass(RenderPass::Screen);
    ...
    memcpy_ubo_buffer(proj_ubo_buffer, &ortho_proj);

    UIVertex* vertex_buffer = (UIVertex*)this->vertex_buffer.mapped;
    uint* index_buffer = (uint*)this->index_buffer.mapped;
    
    uint frame_index = get_frame_index();

    u64 vertex_offset = vertex_arena[frame_index].base_offset;
    u64 index_offset = index_arena[frame_index].base_offset;
    u64 vertices_rendered = 0;
    
    bind_vertex_buffer(cmd_buffer, this->vertex_buffer.buffer, vertex_offset);
    bind_index_buffer(cmd_buffer, this->index_buffer.buffer, index_offset);
    ...
    for (uint i = 0; i < MAX_UI_TEXTURES; i++) {
        ...
        combined_samplers.values[i].texture = dummy_tex;
    }
    ...
    for (const UICmdBuffer& cmd_list : data.layers) {
        for (const UICmd& cmd : cmd_list.cmds) {
            ...
            if (cmd.texture.id > 100) combined_samplers[cmd.texture.id].sampler = sampler;
        }
    }

    {
        ...
        update_descriptor_set(descriptor[frame_index], descriptor_desc);

        bind_descriptor(cmd_buffer, 0, descriptor[frame_index]);
    }
