...
bool handle_events(UI& ui, UI_GID id, UIElementGeo& geo, Events& events) {
    ...
    static hash_set<UI_GID, 13> hovered;
...
void click_text(UI& ui, LayedOutUIView& view, FontInfo& font, glm::vec2 inner_pos, UI_GID id, char* buffer) {
    Input& input = ui.input;
...
bool LayedOutInputString::render(UI& ui, LayedOutUIView& parent) {
    ...
    if (input.type == InputString::StringBuffer) {
        input.sbuffer->reserve(input.max);
        buffer = input.sbuffer->data;
    }
    if (input.type == InputString::SString) {
        buffer = input.ssstring->data;
    }
...
bool draw_input_number(UI& ui, LayedOutUIView& view, UI_GID id, Color hover_color, Color cursor_color, FontInfo& font, char* buffer, bool* incr, bool* decr, float* dragged) {
    Input& input = ui.input;
...
void LayedOutInputString::loose_focus(UI& ui) {
    ...
    if (input.type == InputString::StringBuffer) *input.sbuffer = ui.buffer;
    if (input.type == InputString::SString) *input.ssstring = ui.buffer;
}
