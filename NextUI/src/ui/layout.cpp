...
LayedOutUIView& Empty::compute_layout(UI &ui, const BoxConstraint& constraint) {
    LayedOutUIView& result = alloc_layed_out_view<LayedOutUIView>(ui,this);
    result.geo.extent = {constraint.min,constraint.min};
    result.geo.inner = {};
    return result;
}
...
void compute_flex_layout(UI& ui, LayedOutUIContainer& result, LayoutStyle& layout, Size spacing_size, uint axis, uint alignment, const BoxConstraint& constraint, slice<UIView*> children) {
    ...
    LayedOutUIView** layout_children = alloc_t<LayedOutUIView*>(ui.curr_allocator, children.length);
...
LayedOutUIView& ScrollView::compute_layout(UI& ui, const BoxConstraint& constraint) {
    ...
    LayedOutUIView** layed_out_children = alloc_t<LayedOutUIView*>(ui.curr_allocator, children.length);
